...
| IsOnPrintMenu | 真偽値 | - | 卓上メニューブック（印刷物）に載せる定番品かどうか。`false`（既定）は印刷メニュー用テキスト生成（REQ-055）の対象外。`IsActive`（会計上の販売可否）とは別軸の値で、販売中でも印刷メニューには載せない商品を許容する |
| PrintDisplayName | 文字列 | - | 印刷メニュー上の表記名。空欄の場合は`MenuName`をそのまま使う（会計上の商品名と印刷用の呼び方を分けたい場合に使う） |
| PrintDescription | 文字列 | - | 印刷メニュー用の一行説明・キャッチコピー。任意項目 |
