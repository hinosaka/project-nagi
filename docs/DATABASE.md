...
| FixedCost | 固定費マスタ（予算管理・目標逆算用） | REQ-045 |
| BudgetSettings | 予算設定（目標利益率） | REQ-045 |
...
### 3.11 FixedCost（固定費マスタ）

> 予算管理画面（SCR-012）の「固定費から逆算」で使う、項目別の固定費一覧。家賃改定等があれば都度上書きする現在値のマスタで、月ごとの履歴は持たない

| カラム名 | 型 | 必須 | 説明 |
|---|---|---|---|
| FixedCostId | 文字列 | ○ | 主キー。形式：`FC-0001` |
| Name | 文字列 | ○ | 項目名（例：家賃、人件費） |
| Amount | 数値 | ○ | 月額（円） |

- 関連：他シートとの結合はない。合計額を目標売上逆算の計算に使う（3.12参照）

### 3.12 BudgetSettings（予算設定）

> 予算管理画面（SCR-012）の目標利益率のみを保持する、単一行の設定シート。将来他の設定値が増えても列を追加する形で対応する想定

| カラム名 | 型 | 必須 | 説明 |
|---|---|---|---|
| TargetProfitRate | 数値 | ○ | 目標利益率（%の整数値。例：15は15%） |

- 関連：他シートとの結合はない。目標売上の逆算式（固定費合計÷(1−原価率−目標利益率)）で使う。原価率は`SalesDetail`の全期間実績（原価合計÷売上合計）から算出する
