...
var PrintMenuTextBuilder = {
  ...
  // 商品名の一部と誤読する等）よう、テキスト冒頭に凡例を必ず付ける。この凡例は生成される画像の
  // レイアウト指示も兼ねており、「商品の下に小文字で」の部分は元データの改行位置ではなく、
  // AIが実際にメニュー画像を描く際の一言説明の配置（商品名の下に小さく添える）を指定している
  INTRO_: '以下は卓上メニューの品目リストです。【】は大分類、◆は中分類、各行は「商品名　価格円 - 一言説明」の形式です（一言説明が無い商品もあります。例：生ビール　580円 - よく冷えてます）。商品名・価格は正確に反映し、一言説明は商品の下に小文字で簡潔に記載してください。',
