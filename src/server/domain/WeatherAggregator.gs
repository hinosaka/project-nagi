// ドメイン層：天候別分析（データ分析画面の「天候別分析」タブ）の集計ロジック（Spreadsheet非依存）。
// 天候はBusinessDay.Weatherの事後入力のみで、当日・翌日の予報を持つ手段がないため、この分析は
// 過去の傾向把握（例：雨の日は温かい料理が伸びる）を目的とし、当日の仕入れ予測には使わない前提とする
var WeatherAggregator = {
  // businessDayByDate: {"yyyy-MM-dd": BusinessDayレコード} のMap（呼び出し側で1回だけ構築して渡す）
  aggregate: function (salesInRange, allDetails, businessDayByDate) {
    var totals = {};
    var order = [];
    ...
    return order.map(function (weather) {
